...
const Register = () => {
    const { loginState, dispatch } = useContext(AppContext)
    const isLogin = loginState.isLogin

    const responseSuccess = (res: any) => {
        dispatch({ type: 'name', data: res.profileObj.name })
        dispatch({ type: 'email', data: res.profileObj.email })
        dispatch({ type: 'isLogin', data: true })
        router.push('content')
        refreshSetToken(res)
    }
    ...
    const responseFailure = (res: any) => {
        ...
        dispatch({ type: 'isLogin', data: false })
    }
    ...
    return (
        <>
            <div className=" w-72 bg-background text-primary rounded-2xl p-6 my-6">
                <p> E-mail</p>
                <input
                    className="w-full border-second border-2 p-2 mb-2"
